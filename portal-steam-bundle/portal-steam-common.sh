...
portal_steam_ensure_libvpx() {
	...
	portal_steam_log "libvpx.so.6 not in runtime — downloading libvpx6 for ARM64..."
	deb="${STEAM_DIR}/.cache/libvpx6-arm64.deb"
	mkdir -p "${STEAM_DIR}/.cache"
	wget -c -t 5 -O "${deb}" \
		"http://ports.ubuntu.com/pool/main/libv/libvpx/libvpx6_1.8.2-1ubuntu0.4_arm64.deb" || \
